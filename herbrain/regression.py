...
from strings import cp_str, template_str, residual_str, residual_str_spline
...
def main(
    ...
    lddmm.spline_regression(source=data_set[0]['shape'], targets=data_set,
                            output_dir=regression_dir, times=times.tolist(), subject_id=[''],
                            t0=min(times), target_weights=target_weights, **all_spline_args)
...
def compute_r2(data_set, out_dir, structure, config_id, registration_args):
    ...
    lddmm.deterministic_atlas(
        ...
        structure, atlas_dir, initial_step_size=1e-1, **registration_args)
